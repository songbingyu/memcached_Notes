 ...
 */
//这段代码上述已经比较清楚了。。。一句话让我一次分配够，浪费点就浪费吧
static void conn_init(void) {
    /* We're unlikely to see an FD much higher than maxconns. */
	
	//注意：配的ＦＤ　都是最小的，这个原理涉及到内核这里就不赘述了
	//所以这里可以看做是当前可用最小的了
    int next_fd = dup(1);
...
// 		- I 重写每个数据页尺寸。调整数据项最大尺    /* process arguments */
	...
	//进程使用的部分或者全部的地址空间锁定在物理内存中，防止其被交换到swap空间
	//http://baike.baidu.com/view/9381703.htm
	...
	//各种初始化，可以参考 http://blog.csdn.net/lcli2009/article/details/21525839
